...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    while True:
        ...
        # Select the token with maximum probability ( because it is a greedy search)
...
def run_validation(model,validation_ds,tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state,writer, num_examples=2):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            # assert checks if the batch size is 1, else it will raise AssertionError
            ...
            # Pring to the console ( not using print function, as tqdm progress bar will be interfered)
            print_msg("-"*console_width)
            print_msg(f'SOURCE: {source_text}')
            print_msg(f'TARHET: {target_text}')
...
def get_ds(config):
    ...
    print(f'Max length of sourch sentence: {max_len_src}')
...
def train_model(config):
    ...
    for epoch in range(initial_epoch,config['num_epochs']):
        ...
        for batch in batch_iteration:
            ...
            batch_iteration.set_postfix({f'loss': f'{loss.item():6.3f}'}) # this will add loss in the preogress bar
